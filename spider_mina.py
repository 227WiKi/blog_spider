...
def get_img(i):
    ...
    try:
        ...
        with open(os.getcwd()+"/mina/images/"+filename, "wb+") as f:
...
def get_link(links):
    ...
    l= 'https://files.227wiki.eu.org/d/blog/mina/' + filename
    return l
if __name__ == "__main__":
    for i in tqdm(range(1)):
        ...
        for j in tqdm(range(len(title))):
            name="mina-"+day[j]+'-'+title[j]
            md=hashlib.md5(name.encode(encoding='UTF-8')).hexdigest()
            with open(os.getcwd()+"/mina/"+md+".md","w",encoding='utf-8') as f:
                f.write("---\n")
