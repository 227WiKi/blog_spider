...
def get_img(i):
    ...
    try:
        ...
        with open(os.getcwd()+"/luna/images/"+filename, "wb+") as f:
...
def get_link(links):
    ...
    l= 'https://files.227wiki.eu.org/d/Backup/Blog/luna/' + filename
    return l
if __name__ == "__main__":
    for i in tqdm(range(2)):
        ...
        for j in tqdm(range(len(title))):
            name="luna-"+day[j]+'-'+title[j]
            md=hashlib.md5(name.encode(encoding='UTF-8')).hexdigest()
            with open(os.getcwd()+"/luna/"+md+".md","w",encoding='utf-8') as f:
                f.write("---\n")
